...
The gotcha: there's no singel endpoint that returns full job objects. You get an ID list, then fan out N requests to fetch each item. This shapes your whole data-fetching strategy — you need pagination/batching or you'll fire off hundreds of requests on load.
